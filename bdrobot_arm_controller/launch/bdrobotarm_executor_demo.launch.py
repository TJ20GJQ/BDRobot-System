...
def generate_launch_description():
    ...
    mygazebo_moveit_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(bringup_dir, 'launch', 'moveit_mygazebo.launch.py')
        ),
        launch_arguments=[
    ...
    # 添加demo节点
    bdrobot_arm_executor_demo_node = TimerAction(
        period=5.0,  # 5秒延时
        actions=[
            Node(
                package="bdrobot_arm_controller",
                executable="bdrobot_arm_executor",
                name="bdrobot_arm_executor",
                output="screen",
                parameters=[  # 节点参数
    ...
    ld.add_action(bdrobot_arm_executor_demo_node)
